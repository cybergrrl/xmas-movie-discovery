...
def scrape_film(film_html, not_found):
    ...
    # Find the TMDB ID if it exists
    try:
        tmdb_link = film_soup.find('a', attrs={'data-track-action': 'TMDB'})
        if tmdb_link and 'href' in tmdb_link.attrs:
            tmdb_url = tmdb_link['href'].rstrip('/')
            tmdb_id = tmdb_url.split('/')[-1]
            # Extract type (movie/tv) from URL
            tmdb_type = tmdb_url.split('/')[-2] if len(tmdb_url.split('/')) > 1 else 'movie'
            film_dict["TMDB_ID"] = tmdb_id
            film_dict["TMDB_type"] = tmdb_type
        else:
            film_dict["TMDB_ID"] = not_found
            film_dict["TMDB_type"] = not_found
    except Exception:
        film_dict["TMDB_ID"] = not_found
        film_dict["TMDB_type"] = not_found
